...
class Lexer:
    ...
    def handle_indentation(self):
        ...
        if count > current:
            ...
            while count < self.indent_stack[-1]:
                ...
                tokens.append(Token(TokenType.DEDENT, "", self.line, start_col,value="DEDENT"))
    ...
    def handle_string_literal(self, quote_type, start_col, start_line, prefix=None):
        ...
        if prefix:
            match prefix:
                ...
                case 'f':
                    pass
                case _:
                    return Token(TokenType.ERROR, f"Unknown string prefix {prefix}", start_line, start_col, value=f"Unknown string prefix {prefix}")
        ...
        while True:
            ...
            # EOF
            if c == '\0':
                return Token(TokenType.ERROR, "Unterminated string literal", start_line, start_col, value="Unterminated string literal")
            ...
            if c == '\\' and not skip_escape:
                ...
                if esc in ESCAPE_MAP:
                    ...
                    return Token(TokenType.ERROR, f"Invalid escape \\{esc}", self.line, self.column, value=f"Invalid escape \\{esc}")
    ...
    def next_token(self):
        ...
        if c == '\0':
            if len(self.indent_stack) > 1:
                self.indent_stack.pop()
                return Token(TokenType.DEDENT, "", self.line, self.column, value="DEDENT")
        ...
        # 11. error fallback
        self.advance()
        return Token(TokenType.ERROR, c, self.line, start_col, value=f"Unknown character: {c}")
