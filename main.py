...
# testcaes_dir = os.path.join(os.path.dirname(__file__), "tests", "lexer")

# test_files = [f for f in os.listdir(testcaes_dir) if f.endswith(".lpy")]
# print(f"Found test files in {testcaes_dir}: {test_files}")

# test_files_path = [os.path.join(testcaes_dir, f) for f in test_files]
# print(f"Test file paths: {test_files_path}")
...
#         tokens_dict = [tok.to_dict() for tok in tokens]
        
#         output_file_path = path+".snap"
#         with open(output_file_path,"w") as exp_file:
#             json.dump(tokens_dict, exp_file, indent=4)
